...
song = AudioSegment.from_file("D:/BRUNO-2/boo/background.mp3")
som = AudioSegment.from_file("D:/BRUNO-2/boo/sound.mp3")
...
janela.configure(bg="#4f4312")  # Define a cor de fundo como preto
janela.iconbitmap("images.ico")

# Definir a fonte instalada
fonte_halloween = ("Help Me", 18)  # Use o nome exato da fonte após instalá-la no sistema

# Configuração dos textos com a fonte de Halloween
lbl_instrucoes = tk.Label(janela, text="cLIqUe No FaNtAsMa PaRa GaNhAr PoNtOs!", fg="red", bg="#4f4312", font=fonte_halloween)
lbl_instrucoes.pack()

lbl_pontuacao = tk.Label(janela, text="PONTOS: 0", fg="red", bg="#4f4312", font=fonte_halloween)
lbl_pontuacao.pack()

# Botão de início com a fonte de Halloween
btn_iniciar = tk.Button(janela, text="⏯️ INICIAR JOGO", command=iniciar_jogo, fg="orange", bg="#5f4312", font=fonte_halloween, width=15, height=2)
btn_iniciar.pack(pady=10)

# Carregar a imagem do fantasma
imagem_fantasma_original = Image.open("bixo.png").convert("RGBA")
...
lbl_fantasma = tk.Label(janela, image=imagem_fantasma, bg="#4f4312")
